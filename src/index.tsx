...
import TBodyWrapper from './components/tbody-wrapper';
...
export default class InfiniteScroll extends Component<Props, State> {
  ...
  private _infScroll: HTMLDivElement | HTMLTableSectionElement | undefined;
  ...
  private setScroll = (
    iScroll: HTMLDivElement | HTMLTableSectionElement | undefined
  ) => {
    this._infScroll = iScroll;
  };
  private getScrollContainer = (cType: any) => {
    const CType = cType;
    const style = {
  ...
  render() {
    return this.getScrollContainer(
      this.props.tbody ? TBodyWrapper : DivWrapper
    );
  }
}
